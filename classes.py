	...
	def PlatimZaFotoCallback(self, srvc, pic):
		...
		try:
			...
			try:
				...
				postData = [(self.imageHostersConfig.get(srvc, 'filePostField'), (self.curl.FORM_FILE, pic)), ('PHPSESSID', session)]
				for equals in str(self.imageHostersConfig.get(srvc, 'additionalPostData')).split('&'):
					postData.append(tuple(item for item in equals.split('=')))
				self.postFile(self.imageHostersConfig.get(srvc, 'postUrl'), postData)
				tst = self.buf.getvalue()
				try:
					link = 'http://platimzafoto.ru/' + re.search('"pic_id":"(\w+)"', self.buf.getvalue()).group(1) + '.html'
					self.picsList.append(link)
...
# /callbacks
	...
	def getSlListOld(self):
		self.mysqlCursor.execute("SELECT `id`, `title_en`, `modified`, `screenlist` FROM `f3x_posts` WHERE `id` < 15986 AND `screenlist` LIKE '%picsee.net%' AND `repix` = 1 ORDER BY `id` DESC LIMIT 0,3000")
					...
					try:
						...
						try:
							...
							if not re.search("Content-Length:0", page):
								...
								picName = timestamp + "_sl_" + re.sub("( |:|,|'|\!|\&|\?)", "_", str(data[1]).lower())[:30] + ".jpeg"
								try:
	...
	def ftpPixDownlowder(self):
		...
		for section in self.siteFtpConfig.sections():
			...
			for data in self.mysqlCursor.fetchall():
				...
				self.ftp.login(self.siteFtpConfig.get(self.section, 'user'), self.siteFtpConfig.get(self.section, 'password'))
