    ...
    def repixMe(self):
        ...
        for slData in self.slList.items():
            ...
            if len(slLinks) > 4:
                ...
                print "Updating db..."
                sql = "UPDATE `f3x_posts` SET `repix` = 1, `screenlist` = '{0}' WHERE `id` = {1}".format(slLinks,
                                                                                                         slData[0])
    ...
    def Pic4YouCallback(self, pic, slLinks=list()):
        for tries in range(0, TRIES):
            ...
            for equals in str(self.imageHostersConfig.get(srvc, 'additionalPostData')).split('&'):
                postData.append(tuple(item for item in equals.split('=')))
            try:
                page = self.postFile(self.imageHostersConfig.get(srvc, 'postUrl'), postData, cookies=True).decode(
                    'cp1251')
            except:
